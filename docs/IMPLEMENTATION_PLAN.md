...
**Status:** Audit-driven v2.2 — P0 build hardening is now closed (fonts confirmed as system fonts, no Google Fonts dependency). Early quick-win implementation slices are shipping. Deeper semantic cleanup, locale-surface parity, and factual-governance work still need focused follow-through.
...
- `npm run build` is now **green** — P0 build hardening is closed. Fonts are confirmed as system-UI fonts (no Google Fonts dependency), optional error-tracking is bundler-safe, and the build succeeds reliably on clean environments
...
1. ~~**Build reliability is still not fully hardened**~~ — Resolved. Fonts are system-UI (no Google Fonts), optional error-tracking uses bundler-safe adapter, and build succeeds on clean environments.
...
- ~~non-green build pipeline~~ Resolved — build is green, fonts are system-UI
...
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Production build is not currently reliable.        | Resolved. `npm run build` succeeds reliably on clean environments. Fonts are system-UI fonts (no network-dependent Google Fonts), optional error-tracking uses a bundler-safe adapter, and `next/image` quality config matches usage. | Closed   | Build reliability is no longer a risk.                                                        | Highest — release confidence restored.                                                | Maintain font strategy as system-UI; keep optional integrations on explicit, bundler-safe boundaries.                                 |
...
1. ~~Replace raw `LC` / `EN` / etc. display in quick facts with localized human labels plus code.~~ Resolved: OG/Twitter social images now show `LC — Least Concern` / `LC — Preocupación menor` using `getConservationLabel`. Tree detail structured data and page already used localized labels.
...
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | --------------------------------------------------- | ------------------------------------- |
| Build success rate                      | Resolved: `npm run build` succeeds reliably. System-UI fonts confirmed, no Google Fonts dependency. P0 closed. | 100% on release branch                                                 | Run `npm run build` in clean environment / CI       | Every PR to main                      |
...
1. **Fonts:** Fonts use system-UI stack (no Google Fonts dependency). Confirmed: `--font-geist-sans` resolves to `system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`.
