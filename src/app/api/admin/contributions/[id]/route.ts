...
/* eslint-disable security/detect-object-injection -- route maps controlled action enums to typed statuses and uses parameterized SQL. */
import prisma from "@/lib/prisma";
import { getServerSession } from "next-auth";
import { authOptions } from "@/app/api/auth/[...nextauth]/route";
import { captureApiError } from "@/lib/error-tracking";
import { recalculateReputationForContribution } from "@/lib/contributor-stats";
import type {
  ContributionStatus,
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (["approve", "reject", "implement"].includes(action)) {
      try {
        await recalculateReputationForContribution(id);
