...
/**
 * TreeCardWithFavorite - Tree card with favorite button overlay
 * Client component that extends TreeCard with interactive favorite functionality.
 */
export function TreeCardWithFavorite({ tree }: TreeCardWithFavoriteProps) {
  return (
    <div className="group relative block bg-card rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 border border-primary/10 hover:border-primary/30">
      <Link href={`/trees/${tree.slug}`} className="block">
        <div className="aspect-video bg-primary/10 relative overflow-hidden">
          ...
          {tree.conservationStatus && (
            <span className="absolute top-2 left-2 px-2 py-1 text-xs font-medium bg-secondary/90 text-white rounded">
              {tree.conservationStatus}
            </span>
          )}
        </div>
      ...
      <div className="absolute top-2 right-2 z-10">
        <FavoriteButton
          ...
          locale={tree.locale}
          size="md"
          className="bg-white/80 dark:bg-black/50 backdrop-blur-sm rounded-full shadow-sm hover:bg-white dark:hover:bg-black/70"
        />
      </div>
    </div>
  );
