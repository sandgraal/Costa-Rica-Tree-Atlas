...
<QuickRef
  ...
  items={[
    ...
    { label: "Conservation Status", value: "Not Evaluated (NE)" },
...
_Cryosophila albida_ to **Not Evaluated** rather than Vulnerable in the
public-facing conservation field for this page. That does **not** mean the
species lacks conservation concern in Costa Rica; it means the audit pipeline
is not currently returning a formal IUCN category that supports publishing a
more specific status code here. Until that assessment trail is reviewed again,
the atlas should display **NE** consistently while still documenting habitat
loss, harvesting pressure, and the species' slow recovery dynamics.
